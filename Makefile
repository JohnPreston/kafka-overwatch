...
coverage: ## check code coverage quickly with the default Python
	poetry run coverage run --source kafka_overwatch -m behave tests/features --junit
	poetry run coverage run --source kafka_overwatch -a -m pytest tests/pytests -vv -x
...
codebuild: ## check code coverage quickly with the default Python
	coverage run --source kafka_overwatch -m behave tests/features --junit;\
	BEHAVE=$$? ;\
	coverage run --source kafka_overwatch -a -m pytest tests/pytests -vv -x ;\
	PYTEST=$$? ;\
	echo $$BEHAVE
	echo $$PYTEST
	coverage report -m
...
docs: clean-c9 ## generate Sphinx HTML documentation, including API docs
	rm -f docs/kafka_overwatch.rst
	rm -f docs/modules.rst
	find docs -name "kafka_overwatch.*.rst" -print -delete
	sphinx-apidoc -o docs/ kafka_overwatch
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
conform	: ## Conform to a standard of coding syntax
	isort --profile black kafka_overwatch || poetry run isort --profile black kafka_overwatch
	black kafka_overwatch tests || poetry run black kafka_overwatch tests
	find kafka_overwatch -name "*.json" -type f  -exec sed -i '1s/^\xEF\xBB\xBF//' {} +
